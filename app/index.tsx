...
const DETAIL_TEXT_LINE_REVEAL_MS = 1750;
...
function OracleApp() {
  ...
  const journalPreviewCardWidth = Math.min(
    journalDetailContentWidth * 0.42,
    112,
  );
  ...
  const [selectedJournalEntryId, setSelectedJournalEntryId] = useState<
    string | null
  >(null);
  const [isJournalDetailCardExpanded, setIsJournalDetailCardExpanded] =
    useState(false);
  const [isJournalDetailCardFront, setIsJournalDetailCardFront] =
    useState(true);
  ...
  const [singleCardTransitionSlot, setSingleCardTransitionSlot] = useState<
    number | null
  >(null);
  ...
  const handleConfirmSelection = useCallback(
    (confirmed: boolean) => {
      if (confirmed) {
        ...
        if (drawMode !== "triple") {
          const transitionSlot = selectedSlotRef.current ?? selectedSlot;
          if (transitionSlot !== null && fanLayout) {
            ...
            const animation = Animated.timing(singleCardGrowAnim, {
              toValue: 1,
              duration: SINGLE_CARD_GROW_DURATION_MS,
              easing: Easing.inOut(Easing.cubic),
              useNativeDriver: false,
            });
            singleCardGrowAnimationRef.current = animation;
            animation.start(({ finished }) => {
              singleCardGrowAnimationRef.current = null;
              if (!finished) {
                return;
  ...
  const bannerToFanGapOffset = useMemo(() => 13, []);
  ...
  const singleCardTransitionStyle = useMemo(() => {
    ...
    const slot = fanSlots[singleCardTransitionSlot];
    if (!slot) {
      return null;
    }
    const sourceLeft = fanAreaCenterX + slot.x - fanCardWidth / 2;
    const sourceTop = fanLayout.y + slot.y + fanCardHeight * 0.18;
    ...
    return {
      left: singleCardGrowAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [sourceLeft, targetLeft],
      }),
      top: singleCardGrowAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [sourceTop, targetTop],
      }),
      width: singleCardGrowAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [fanCardWidth, cardWidth],
      }),
      height: singleCardGrowAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [fanCardHeight, cardWidth * CARD_HEIGHT_RATIO],
      }),
      transform: [
        {
          rotate: singleCardGrowAnim.interpolate({
            inputRange: [0, 1],
            outputRange: [`${slot.rot}deg`, "0deg"],
          }),
        },
          ...
          <View style={styles.column}>
            ...
            >
              ...
              >
                          ...
                          >
                            {selectedJournalCard?.image ? (
                              <Image
                                ...
                                style={styles.journalCardPreviewImage}
                              />
                            ) : (
                              <View style={styles.thumbnailFallback} />
                            )}
                ...
                {isJournalDetailCardExpanded ? (
                  <View style={styles.journalCardExpandedOverlay}>
                    <View style={styles.journalCardExpandedBackdrop} />
                    <View style={styles.journalCardExpandedPanel}>
                      <CardFlip
                        ...
                        front={
                          selectedJournalCard?.image ? (
                            <Image
                              source={selectedJournalCard.image}
                              style={styles.journalCardExpandedImage}
                            />
                          ) : (
                            <View style={styles.thumbnailFallback} />
                          )
                        ...
                        back={
                          <ImageBackground
                            ...
                            style={styles.journalCardExpandedBack}
                            imageStyle={styles.cardImage}
                          >
                            <View style={styles.journalCardExpandedBackOverlay}>
                              ...
                              <ScrollView
                                style={styles.journalCardExpandedBackScrollArea}
                                contentContainerStyle={
                                  styles.journalCardExpandedBackScroll
                                }
                                showsVerticalScrollIndicator={false}
                                nestedScrollEnabled
                              >
...
const styles = StyleSheet.create({
  ...
  journalCardExpandedBackdrop: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "rgba(12, 9, 8, 0.55)",
  },
  journalCardExpandedPanel: {
    alignItems: "center",
    gap: spacing.md,
    paddingVertical: spacing.md,
  },
  journalCardExpandedCard: {
    ...shadow.soft,
  },
  journalCardExpandedImage: {
    width: "100%",
    height: "100%",
    resizeMode: "cover",
  },
  journalCardExpandedBack: {
    flex: 1,
  },
  journalCardExpandedBackOverlay: {
    flex: 1,
    paddingVertical: spacing.xl,
    paddingHorizontal: spacing.lg,
    justifyContent: "flex-start",
  ...
  journalCardExpandedBackScrollArea: {
    flex: 1,
  },
  journalCardExpandedBackScroll: {
    paddingHorizontal: spacing.md,
    paddingBottom: spacing.md,
  },
